    ...
    while epoch < max_num_epochs:
        ...
        if epoch % eval_interval == 0:
            ...
            if log_errors == "PerAtomRMSE":
                ...
                logging.info(
                    f"Epoch {epoch}: loss={valid_loss:.4f}, pc={patience_counter}, lr={optimizer.param_groups[0]['lr']:.4f}, RMSE_E_per_atom={error_e:.1f} meV, RMSE_F={error_f:.1f} meV / A"
                )
            elif (
                ...
                logging.info(
                    f"Epoch {epoch}: loss={valid_loss:.4f}, pc={patience_counter}, lr={optimizer.param_groups[0]['lr']:.4f}, RMSE_E_per_atom={error_e:.1f} meV, RMSE_F={error_f:.1f} meV / A, RMSE_stress_per_atom={error_stress:.1f} meV / A^3"
                )
            elif (
                log_errors == "PerAtomRMSEstressvirials"
                ...
                logging.info(
                    f"Epoch {epoch}: loss={valid_loss:.4f}, pc={patience_counter}, lr={optimizer.param_groups[0]['lr']:.4f}, RMSE_E_per_atom={error_e:.1f} meV, RMSE_F={error_f:.1f} meV / A, RMSE_virials_per_atom={error_virials:.1f} meV"
                )
            elif log_errors == "TotalRMSE":
                error_e = eval_metrics["rmse_e"] * 1e3
                error_f = eval_metrics["rmse_f"] * 1e3
                logging.info(
                    f"Epoch {epoch}: loss={valid_loss:.4f}, pc={patience_counter}, lr={optimizer.param_groups[0]['lr']:.4f}, RMSE_E={error_e:.1f} meV, RMSE_F={error_f:.1f} meV / A"
                )
            elif log_errors == "PerAtomMAE":
                error_e = eval_metrics["mae_e_per_atom"] * 1e3
                error_f = eval_metrics["mae_f"] * 1e3
                logging.info(
                    f"Epoch {epoch}: loss={valid_loss:.4f}, pc={patience_counter}, lr={optimizer.param_groups[0]['lr']:.4f}, MAE_E_per_atom={error_e:.1f} meV, MAE_F={error_f:.1f} meV / A"
                )
            elif log_errors == "TotalMAE":
                error_e = eval_metrics["mae_e"] * 1e3
                error_f = eval_metrics["mae_f"] * 1e3
                logging.info(
                    f"Epoch {epoch}: loss={valid_loss:.4f}, pc={patience_counter}, lr={optimizer.param_groups[0]['lr']:.4f}, MAE_E={error_e:.1f} meV, MAE_F={error_f:.1f} meV / A"
                )
            elif log_errors == "DipoleRMSE":
                error_mu = eval_metrics["rmse_mu_per_atom"] * 1e3
                logging.info(
                    f"Epoch {epoch}: loss={valid_loss:.4f}, pc={patience_counter}, lr={optimizer.param_groups[0]['lr']:.4f}, RMSE_MU_per_atom={error_mu:.2f} mDebye"
                )
                ...
                error_mu = eval_metrics["rmse_mu_per_atom"] * 1e3
                logging.info(
                    f"Epoch {epoch}: loss={valid_loss:.4f}, pc={patience_counter}, lr={optimizer.param_groups[0]['lr']:.4f}, RMSE_E_per_atom={error_e:.1f} meV, RMSE_F={error_f:.1f} meV / A, RMSE_Mu_per_atom={error_mu:.2f} mDebye"
                )
            if log_wandb:
                wandb_log_dict = {
                    "epoch": epoch,
            ...
            if valid_loss >= lowest_loss:
                ...
                if swa is not None:
                    ...
                    elif patience_counter >= swa.num_epochs:
                        logging.info(
